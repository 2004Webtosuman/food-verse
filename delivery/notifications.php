...
<body class="bg-app-bg min-h-screen pb-32">
    ...
    <div class="p-6 max-w-lg mx-auto space-y-4">
        <?php if (empty($notifications)): ?>
            <div class="flex flex-col items-center justify-center py-20 text-gray-400">
                ...
                <p class="text-xs text-gray-400 mt-2">When a delivery is assigned, it will appear here.</p>
            </div>
        <?php else: ?>
            <?php foreach ($notifications as $n): ?>
                <a href="<?php echo htmlspecialchars($n['link'] ?? '#'); ?>" class="block bg-white p-5 rounded-[2rem] border border-card-border shadow-sm hover:shadow-md hover:border-primary/30 transition-all relative group">
    ...
    <!-- Need the safe area padding so bottom nav doesn't cover -->
